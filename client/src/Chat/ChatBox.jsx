...
const ChatBox = (props) => {
	...
	useEffect(() => {
		const reloadMessages = async () => {
			if (props.scope === "Global Chat") {
				const res = await getGlobalMessages();
				setMessages([...messages, ...res]);
			} else if (props.scope !== null && props.conversationId !== null) {
				const res = await getConversationMessages(props.user._id);
				if (res) {
					setMessages([...messages, ...res]);
				} else {
					setMessages([]);
	...
	}, [lastMessage, props.scope, props.conversationId]); // eslint-disable-line react-hooks/exhaustive-deps
	...
	const renderMessage = messages.map((m) => (
		...
		>
			<ListItemAvatar className={classes.avatar}>
				<Avatar>{commonUtilites.getInitialsFromName(m.fromObj[0].name)}</Avatar>
			</ListItemAvatar>
			<ListItemText
				classes={{
			...
			<Grid item xs={12}>
				<Grid container className={classes.messageContainer}>
					<Grid item xs={12} className={classes.messagesRow}>
						{messages && <List>{renderMessage}</List>}
						<div ref={chatBottom} />
					</Grid>
